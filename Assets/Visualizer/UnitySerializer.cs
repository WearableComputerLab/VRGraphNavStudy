...
public class UnitySerializer {  
	...
	public Vector3 DeserializeVector3(byte[] arrayByte)  
	{  
		Vector3 vector3 = new Vector3();  
		vector3.x = BitConverter.ToSingle(arrayByte, index); index += 4;  
		vector3.y = BitConverter.ToSingle(arrayByte, index); index += 4;  
		vector3.z = BitConverter.ToSingle(arrayByte, index); index += 4;  
		return vector3;  
	}  
	//  
	...
	public Quaternion DeserializeQuaternion(byte[] data)  
	{  
		Quaternion quat = new Quaternion();  
		quat.x = BitConverter.ToSingle(data, index); index += 4;  
		quat.y = BitConverter.ToSingle(data, index); index += 4;  
		quat.z = BitConverter.ToSingle(data, index); index += 4;  
		quat.w = BitConverter.ToSingle(data, index); index += 4;  
		return quat;  
	}  
	...
	public int DeserializeInt(byte[] data)  
	{  
		int i = BitConverter.ToInt32(data, index); index += 4;  
		return i;  
	}  
	//  
	...
	public byte[] LoadFromFile(string fileName)
	{
		using (FileStream fs = new FileStream(fileName, FileMode.Open, FileAccess.Read))
		{
			...
			BinaryFormatter bf = new BinaryFormatter();
			byte[] result = (byte[])bf.Deserialize(fs);		
