...
namespace UnityStandardAssets.ImageEffects
{
    ...
    public class VignetteAndChromaticAberration : PostEffectsBase
    {
        ...
        void OnRenderImage (RenderTexture source, RenderTexture destination)
        {
            ...
            float widthOverHeight = 0.5f;
            const float oneOverBaseSize = 1.0f / 2000;
            ...
            if (doPrepass)
            {
                ...
                {
                    color2A = RenderTexture.GetTemporary (rtW / 2, rtH, 0, source.format);

                    Graphics.Blit (source, color2A, m_ChromAberrationMaterial, 0);

                    for(int i = 0; i < 2; i++)
                    {	// maybe make iteration count tweakable
                        m_SeparableBlurMaterial.SetVector ("offsets",new Vector4 (0.5f, blurSpread * oneOverBaseSize, 0.0f, 0.0f));
                        RenderTexture color2B = RenderTexture.GetTemporary (rtW / 2, rtH, 0, source.format);
                ...
                m_VignetteMaterial.SetFloat("_Intensity", (2 / (1.0f - intensity) - 1.0f));		// intensity for vignette
                m_VignetteMaterial.SetFloat("_Blur", (1.0f / (1.0f - blur)) - 1.0f);					// blur intensity
                m_VignetteMaterial.SetTexture ("_VignetteTex", GetComponent<MeshRenderer>().material.mainTexture);  // blurred texture
			...
			m_ChromAberrationMaterial.SetTexture("_mainTex", GetComponent<MeshRenderer>().sharedMaterial.mainTexture);	// blurred texture
